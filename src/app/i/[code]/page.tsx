...
const DESCRIPTION = `הוזמנתם ל${SITE_NAME} — ${SITE_TAGLINE}. נרשמים, מקבלים ₪10,000 וירטואליים, ומתחילים לחזות סקרים, קואליציות ומהלכים פוליטיים.`;
        ...
        <div className="relative p-5 sm:p-8">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <div className="truncate text-sm font-bold text-white">{inviter.name ?? "אחד הסוחרים"}</div>
              <div className="text-xs text-white/70">הזמינו אתכם לשוק החיזויים</div>
            </div>
          </div>
          ...
          <p className="mt-2 text-sm leading-relaxed text-white/75 sm:text-lg">
            ...
            מקבלים <strong className="text-white">{money(STARTING_BALANCE)} וירטואליים</strong>, ומתחילים לחזות.
          </p>
          <div className="mt-5 flex flex-col gap-2 sm:flex-row">
            {alreadyIn ? (
              ...
              <>
                ...
                >
                  הצטרפות · {money(STARTING_BALANCE)} וירטואליים
                </Link>
                <Link
                  href="/"
                  className="tap pressable inline-flex items-center justify-center rounded-xl border border-white/35 bg-white/10 px-5 py-3 font-semibold text-white hover:bg-white/20"
      ...
      {alreadyIn ? (
        ...
        <section className="card p-4 sm:p-5">
          ...
          <p className="mt-1.5 text-sm leading-relaxed text-muted">
            שוק חיזויים בסגנון Polymarket על הבחירות לכנסת ה־26, <strong className="text-text">בכסף וירטואלי בלבד</strong> —
            בלי הימורים, בלי כסף אמיתי. קונים ״כן״ או ״לא״ על שאלה, המחיר הוא ההסתברות שהשוק נותן לה, וכל מניה מנצחת שווה
            ₪1 וירטואלי. אחרי ההרשמה תקבלו גם קישור אישי משלכם, שמזכה אתכם ב{money(REFERRAL_BONUS)} על כל חבר/ה שתביאו.
          </p>
