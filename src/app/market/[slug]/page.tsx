  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px] lg:gap-6">
      <div className="space-y-4 sm:space-y-5">
        <nav className="-my-1 text-xs text-muted">
        ...
        <section className="card p-3.5 sm:p-5">
          ...
          {market.tags.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-1.5">
              {market.tags.map((t) => (
                <Link key={t} href={`/?q=${encodeURIComponent(t)}`} className="rounded-full bg-surface-2 px-2.5 py-1.5 text-xs text-muted hover:text-text">
                  #{t}
                </Link>
              ))}
            </div>
