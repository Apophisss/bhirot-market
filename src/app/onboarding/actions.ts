...
/**
 * "לא עכשיו" על גבי ההצעה למלא את השאלון. זו דחייה, לא תשובה: היא נשמרת בחשבון
 * לשבוע (src/lib/survey-offer.ts) ולא כותבת תשובה לשאלון, כדי שמשתמש שכבר רשום — מי
...
export async function snoozeSurvey() {
  ...
  await snoozeSurveyPrompt(session.user.id);
