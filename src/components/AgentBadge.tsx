...
export async function AgentBadge({ resolvedOnBoard = 0 }: { resolvedOnBoard?: number }) {
  ...
  return (
    <Link
      href="/about#updates"
      className="inline-flex items-start gap-2 rounded-2xl border border-white/25 bg-white/10 px-3 py-1.5 text-[13px] leading-relaxed text-white/80 backdrop-blur transition hover:border-white/50 hover:bg-white/15 sm:items-center sm:rounded-full sm:text-xs"
      title={last?.summary}
    >
