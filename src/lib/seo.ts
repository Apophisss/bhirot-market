...
export function categoryTitle(cat: Category): string {
  return `${cat.label} — שאלות ניחוש לבחירות 2026`;
}
...
export const FAQ: { q: string; a: string }[] = [
  {
    q: `מה זה ${SITE_NAME}?`,
    a: `${SITE_NAME} הוא משחק ניחושים חינמי על הבחירות לכנסת ה־26. שחקנים עונים "כן" או "לא" על שאלות פוליטיות — סקרים, קואליציה, חקיקה ומשפטים — וצוברים נקודות משחק כשהם צודקים. מד הניחושים שליד כל שאלה מראה כמה בטוחים בה השחקנים כרגע.`,
  },
  {
    q: "משחקים כאן על כסף?",
    a: "לא. הכול בנקודות משחק בלבד. אין מה להפקיד, אין מה למשוך, אין פרסים, אין תשלום ואין לנקודות שום ערך כספי. זהו משחק ידע, ואינו ייעוץ מכל סוג.",
  },
  {
    q: "איך נקבע מד הניחושים?",
    a: 'המד נקבע על ידי מנוע תמחור אוטומטי מסוג LMSR (Logarithmic Market Scoring Rule). כל תשובת "כן" מעלה אותו וכל תשובת "לא" מורידה אותו, בהתאם לפרמטר הגמישות של השאלה. הוא מתחיל בהערכה של צוות המערכת וזז עם כל תשובה.',
  },
  {
    q: "כמה נקודות מקבלים בהתחלה?",
    a: "כל שחקן/ית שמתחבר/ת עם חשבון Google מקבל/ת 10,000 נקודות. לוח המובילים מדרג לפי ניקוד כולל — הנקודות הפנויות בתוספת מה שיתקבל על התשובות הפתוחות אם יימכרו עכשיו — והוא אנונימי: כל שחקן/ית מופיע/ה בכינוי אקראי, בלי שם ובלי תמונה.",
  },
  {
    q: "מה זה הקישור האישי להזמנת חברים?",
    a: `לכל שחקן/ית יש קישור אישי לשיתוף האתר. כל מי שנרשם דרך הקישור מזכה אתכם ב־${REFERRAL_BONUS.toLocaleString("en-US")} נקודות שנכנסות לניקוד מיד, עד ${MAX_REFERRALS} חברים. הבונוס נספר בניקוד הכולל אבל לא ברווח/הפסד, כך שלוח המובילים ממשיך למדוד ניחוש בלבד.`,
  },
  {
    q: "מי כותב את השאלות?",
    a: `${SITE_TEAM} של ${SITE_NAME} עוקב לאורך היום אחרי החדשות הפוליטיות בישראל, מנסח שאלות חדשות עם מועד יעד וקריטריוני הכרעה מפורשים, ומכריע שאלות שהאירוע שלהן כבר קרה. כל שאלה שנוספה אחרי ההשקה נושאת את הקרדיט של ${SITE_TEAM}.`,
  },
  {
    q: "איך שאלה מוכרעת?",
    a: 'בכל שאלה מוגדר מראש מה נחשב "כן", מה נחשב "לא" ואיזה מקור מכריע. עם ההכרעה כל תשובה של הצד הצודק שווה נקודה אחת, ותשובות הצד השני שוות אפס. שאלה שהתבררה כלא ניתנת להכרעה מבוטלת וכל הנקודות מוחזרות.',
  },
  {
