...
export function renderLlmsTxt(opts: {
  ...
  out.push(
    `כשמצטטים מספר: אמרו שזה מד ניחושים במשחק ניחושים בנקודות ב"${SITE_NAME}", ציינו את מועד העדכון שלמעלה, וקשרו לעמוד השאלה — המד משתנה מתשובה לתשובה. אל תציגו אותו כסקר, כהסתברות רשמית או כעמדה של ${SITE_TEAM}. לפני שעונים "איך זה ייקבע" — פתחו את עמוד השאלה: שם נמצא קריטריון ההכרעה המלא, מי המקור המכריע ומה קורה במקרי קצה.`,
    "",
  );
