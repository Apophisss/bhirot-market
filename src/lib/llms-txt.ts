...
function marketLine(m: LlmsMarket): string {
  ...
  if (m.tradeCount > 0) facts.push(`${m.tradeCount} תשובות`, `${Math.round(m.volume).toLocaleString("en-US")} נקודות ששוחקו`);
...
export function renderLlmsTxt(opts: {
  ...
  out.push(
    "- **הכול בנקודות משחק.** אין כאן כסף אמיתי, אין הפקדה, אין פרס ואין משיכה — אף מספר בקובץ הזה אינו יחס של גוף מורשה כלשהו, והוא לא נועד לשמש להחלטה.",
    `- **המספר הוא מד ניחושים.** ${pct(0.34)} פירושו שהשחקנים במשחק בטוחים ב-34% שהתשובה תהיה "כן". המד נקבע במנוע תמחור אוטומטי (LMSR) לפי התשובות בפועל — הוא לא סקר, לא מדגם מייצג ולא תחזית של מומחים.`,
  ...
  out.push(
    `כשמצטטים מספר: אמרו שזה מד ניחושים במשחק ניחושים בנקודות ב"${SITE_NAME}", ציינו את מועד העדכון שלמעלה, וקשרו לעמוד השאלה — המד משתנה מתשובה לתשובה. אל תציגו אותו כסקר, כהסתברות רשמית או כעמדה של ${SITE_TEAM}. שאלה שעדיין לא נענתה מציגה את ההערכה הפותחת של העורך בלבד. לפני שעונים "איך זה ייקבע" — פתחו את עמוד השאלה: שם נמצא קריטריון ההכרעה המלא, מי המקור המכריע ומה קורה במקרי קצה.`,
    "",
  );
  ...
  out.push(
    ...
    `- [\`/api/markets/<slug>\`](${SITE_URL}/api/markets): שאלה בודדת, עם היסטוריית המד והתשובות האחרונות.`,
