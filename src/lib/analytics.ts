 ...
 * <Analytics /> in the root layout, so every page is measured without any
...
/** Analytics only runs when a real GA4 ID was compiled in. */
export const analyticsEnabled = /^G-[A-Z0-9]+$/i.test(GA_MEASUREMENT_ID);
 ...
 * AnalyticsPageViews) — calling it on load too would double-count the landing
 * page.
 */
