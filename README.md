...
| מספרי הפעילות בשאלה | הנפח ומספר העסקאות שכל כרטיס ועמוד שאלה מציגים, וכן רשימת ״עסקאות אחרונות״ והתגובות שמתחתיה, הם **תצוגה בלבד** (`src/lib/fake-market-stats.ts`, `src/lib/fake-comments.ts`): נגזרים מזהה השאלה ומגילה, לא נכתבים למסד, ולא נוגעים במחיר, בתיק או בלוח המובילים. `MarketView.volume`/`tradeCount`, לוח הניהול, `/api/health`, באנדל הניתוח וההכרעות ממשיכים לקרוא את המספרים האמיתיים |
...
src/lib/fake-comments.ts      שרשור התגובות שמתחת לכל שאלה (תצוגה בלבד, בכינויים של לוח המובילים)
