...
- בוחרים **פעם אחת** סכום לכל תשובה, **בטווח מחייב של ₪5–₪100** (סליידר + קיצורים). הבחירה נשמרת **בחשבון** ולכן חוזרת בכל מכשיר ובכל דפדפן (ראו ״מה נשמר בחשבון״ למטה); לאורח, שאין לו חשבון, היא נשמרת ב־`localStorage` ומאומצת לחשבון ברגע ההתחברות.
...
| מיון החפיסה ו״כולל שאלות שכבר עניתי״ | `user_setting.rapidSort` / `rapidIncludeAnswered` | זהים |
| ״לא עכשיו״ על הצעת השאלון | `user_setting.surveySnoozedUntil` | דחייה במכשיר אחד משתיקה בשניהם |
| תשובות שניתנו **לפני** ההתחברות | `localStorage` שבדפדפן | נכנסות לחשבון ברגע ההתחברות, באותו דפדפן |
