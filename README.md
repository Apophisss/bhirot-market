...
- **אירועים** (`gaEvent` ב־`src/lib/gtag.ts`): `trade` (קנייה/מכירה בדף שוק), `rapid_answer`
  (תשובה במצב זריז), `comment_post`, ושני אירועי ההזדהות המומלצים של GA4 — `sign_up` ו-`login`,
  עם פרמטר `method` (‏`google` בפרודקשן). הסכום נשלח כפרמטר `amount` ולא כ־`value` עם `currency`:
  הכסף כאן וירטואלי, ו־`value` היה מכניס אותו לדוחות ההכנסות של GA כאילו אינו.
