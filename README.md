...
| שאלון והמלצות | שאלון פוליטי קצר (`/onboarding`) לכל מי שמתחבר — נושאים, מתמודדים וקצב — שקובע את ההמלצות הראשונות (`src/lib/preferences.ts`) |
...
## שאלון ההיכרות וההמלצות (`/onboarding`)

משתמש חדש הוא דף חלק: אין לו היסטוריית מסחר, ולכן בלי שום קלט הלוח הראשון שהוא רואה מסודר לפי מה
שכל השאר עושים. השאלון הקצר הוא התחליף לזה.

- **מי רואה אותו** — כל מי שמתחבר מועבר אליו מיד אחרי ההתחברות (`/login` שולח ל-`/onboarding?next=<היעד>`).
  מי שכבר ענה מועבר הלאה בלי לראות אותו שוב. משתמשים ותיקים שנרשמו לפני שהשאלון עלה מקבלים כרטיס
  הזמנה בראש הלוח, עם כפתור ״לא עכשיו״.
...
- **דילוג** — כפתור ״דילוג — תראו לי הכול״ שומר תשובה ריקה (`status: "skipped"`), כך שאף אחד לא נשאל פעמיים.
- **שינוי** — ״ההעדפות שלי״ בתפריט המשתמש (`/onboarding?edit=1`).
- **מה זה משנה** — סעיף ״מומלץ בשבילכם״ בראש הלוח (`rankByPreferences`), ותוספת ניקוד לסדר של
  ״מצב זריז״ במיון ״מעורבב״ (`preferenceBoost`). התוספת מכוונת להיות נגיעה ולא דריסה: שאלה שנסגרת
  הערב עדיין עולה על שאלה בנושא מועדף שנסגרת בעוד חודשיים.

המשקלים (קטגוריה 3, מתמודד 4, בונוס/קנס לפי הקצב) יושבים כולם ב-`src/lib/preferences.ts` — מודול ללא
תלויות, בדיוק כמו `rapid.ts` — והשאילתות ב-`src/lib/preferences-store.ts`. התשובות נשמרות בטבלה
`user_preference` (שורה אחת למשתמש; עצם קיומה הוא מה שעוצר שאלה חוזרת). כשמשנים את השאלות עצמן
מעלים את `SURVEY_VERSION` וכל מי שענה לגרסה קודמת יישאל שוב.
...
src/lib/preferences.ts   משקלי השאלון וההמלצות (מודול ללא תלויות מסד נתונים)
...
src/app/                 דפים: /, /rapid, /category/[id], /market/[slug], /portfolio, /leaderboard, /activity, /about, /login, /onboarding
```
