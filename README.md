...
4. **DNS**: רשומת `A` **אחת בלבד** של הדומיין (ואופציונלית `www`) אל ה־IP של השרת. Caddy מנפיק תעודת
   Let's Encrypt לבד — אבל Let's Encrypt מגריל רשומה מבין כל ה־A שקיימות, ולכן כל רשומה עודפת שוברת גם את
   הנפקת התעודה וגם את מי שהדפדפן שלו יגריל אותה. הפריסה מדפיסה אזהרה כשהיא רואה יותר מאחת.
