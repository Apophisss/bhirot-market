...
| מספרי הפעילות בשאלה | הנפח ומספר העסקאות שכל כרטיס ועמוד שאלה מציגים, וכן רשימת ״עסקאות אחרונות״, הם **תצוגה בלבד** (`src/lib/fake-market-stats.ts`): נגזרים מזהה השאלה ומגילה, לא נכתבים למסד, ולא נוגעים במחיר, בתיק או בלוח המובילים. `MarketView.volume`/`tradeCount`, לוח הניהול, `/api/health`, באנדל הניתוח וההכרעות ממשיכים לקרוא את המספרים האמיתיים |
...
| גרף המגמה | היסטוריה אמיתית מ־`price_history` + אומדן טרום־מסחר לתצוגה בלבד (`src/lib/synthetic-history.ts`), חסום ל־±3 נק׳ מהמחיר הרשום. `src/lib/display-history.ts` מגיש את הסדרה כקו רציף אחד |
...
src/lib/fake-market-stats.ts  הנפח, מספר העסקאות ורשימת העסקאות של כל שאלה (תצוגה בלבד, נגזר מהזהה ומהגיל)
...
- **`/llms.txt`**: הלוח כולו כקובץ Markdown אחד, לפי המוסכמה של [llmstxt.org](https://llmstxt.org) — עוזרי AI קוראים אותו במקום לרנדר את האתר. ה-sitemap אומר *אילו כתובות קיימות*; הקובץ הזה אומר *מה האתר ומה הוא אומר עכשיו*: שהכסף וירטואלי, שמחיר הוא הסתברות ולא סקר, מי כותב ומי מכריע, ואז השאלות הפתוחות עם המחיר, מועד הסגירה וקריטריון ההכרעה. נבנה בכל בקשה מהנתונים החיים (`src/lib/llms-txt.ts` מחליט מה נכנס, `src/app/llms.txt/route.ts` מגיש). **המספרים בו הם אלה שבלוח** — המחיר, מועד הסגירה, הנפח ומספר העסקאות זהים לאלה שמופיעים בעמוד השאלה שהשורה מקשרת אליו (`MarketView.displayVolume`/`displayTradeCount`), כי מספר שמודל עשוי לצטט למישהו חייב להיות זה שרואים באתר. מונה השאלות המנופח של ההירו וזרם הפעילות ב-`/activity` (`display-stats.ts`, `fake-activity.ts`) לא נכנסים לשם: אין להם עמוד מקביל שאפשר להצביע עליו.
