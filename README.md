...
| לוח ניהול | `/admin` — סטטיסטיקות האתר, פרסום שאלה חדשה, תיבת ההצעות והפניות. פתוח רק לכתובות שב-`ADMIN_EMAILS` |
| קלט ממשתמשים | `/suggest` (הצעת שאלה חדשה) ו-`/contact` (פנייה לצוות) — פתוחים לכולם, גם בלי התחברות |
...
## לוח הניהול (`/admin`) והקלט מהמשתמשים

שלושה דפים חדשים סוגרים את המעגל בין המשתמשים לצוות המערכת:

| דף | מי נכנס | מה עושים בו |
|---|---|---|
| `/suggest` | כל אחד (גם בלי התחברות) | מציעים שאלה חדשה: ניסוח, קטגוריה, מועד יעד, הערכת הסתברות, תמונה וקישור למקור — עם תצוגה מקדימה של הכרטיס כפי שייראה בלוח. מי שמחובר רואה בתחתית הדף את ההצעות שלו ואת מה שקרה להן. |
| `/contact` | כל אחד (גם בלי התחברות) | פנייה לצוות: שאלה, דיווח על טעות בשוק, באג או רעיון. |
| `/admin` | **רק כתובות מייל שמופיעות ב-`ADMIN_EMAILS`** | סטטיסטיקות האתר, פרסום שאלות, וטיפול בתיבה. |
...
הרשימה נקראת ממשתנה סביבה ולא מהמסד — הלוח הוא המקום היחיד שאפשר לפרסם ממנו שאלות ולקרוא בו את כל
הפניות, ולכן אי אפשר להעניק הרשאה מתוך האתר עצמו. מי שמחובר עם כתובת שאינה ברשימה מקבל מסך ״אין הרשאה״,
ושתי ה-API של הלוח מחזירות 401. רשימה ריקה = הלוח סגור לכולם. הדף מסומן `noindex` ונחסם ב-`robots.txt`.

### מה יש בלוח

- **סקירה** — משתמשים (סה״כ, חדשים ביממה/שבוע, פעילים), שווקים לפי סטטוס, מחזור ועסקאות (סה״כ / 24 שעות / שבוע),
  פוזיציות פתוחות ושוויין, גרף מחזור יומי לשבועיים, פילוח לפי קטגוריה, השווקים הנסחרים ביותר, ריצות העדכון האחרונות —
  ובראש הכול **חוב ההכרעות**: שווקים פתוחים שמועד הסגירה שלהם כבר עבר.
- **שאלה חדשה** — טופס פרסום מלא: ניסוח, רקע, כללי הכרעה, קטגוריה, אנשים (מספקים את התמונה), תמונה חלופית, תגיות,
  מקורות, מועד סגירה בשעון ישראל, מחיר פתיחה ונזילות — עם תצוגה מקדימה חיה. הפרסום פותח מסחר מיד, ומחזיר את
  אובייקט ה-JSON להוספה ל-`data/markets.json`, שנשאר מקור האמת.
- **הצעות משתמשים** — כל הצעה עם סטטוס (ממתינה / אושרה / נדחתה), הערה פנימית, וכפתור ״פרסום כשאלה״ שפותח את טופס
  הפרסום כשהוא כבר מלא בפרטי ההצעה. הפרסום מסמן את ההצעה כאושרה ומקשר אותה לשוק שנפתח.
- **פניות** — תיבת ההודעות מ-`/contact`, עם סטטוס (חדש / בטיפול / טופל), הערה פנימית וקישור `mailto` למענה.

השאלות שנוצרות בלוח נכנסות למסד הנתונים בלבד. כדי שישרדו הקמה מחדש של המסד יש להעתיק את ה-JSON שמוחזר
אחרי הפרסום אל `data/markets.json` ולבצע commit — בדיוק כמו כל שאלה של רוטינת העדכון.
...
src/app/                 דפים: /, /rapid, /for-you, /category/[id], /market/[slug], /portfolio, /leaderboard, /activity, /about, /login, /suggest, /contact, /admin
```
