...
- **`/llms.txt`**: הלוח כולו כקובץ Markdown אחד, לפי המוסכמה של [llmstxt.org](https://llmstxt.org) — עוזרי AI קוראים אותו במקום לרנדר את האתר. ה-sitemap אומר *אילו כתובות קיימות*; הקובץ הזה אומר *מה האתר ומה הוא אומר עכשיו*: שהכסף וירטואלי, שמחיר הוא הסתברות ולא סקר, מי כותב ומי מכריע, ואז השאלות הפתוחות עם המחיר, מועד הסגירה וקריטריון ההכרעה. נבנה בכל בקשה מהנתונים החיים (`src/lib/llms-txt.ts` מחליט מה נכנס, `src/app/llms.txt/route.ts` מגיש). **המספרים בו הם האמיתיים** — מונה השאלות המנופח וזרם הפעילות לראווה (`display-stats.ts`, `fake-activity.ts`) לא נכנסים לשם בכוונה: מספר שמודל עשוי לצטט למישהו חייב להיות זה שבלוח.
