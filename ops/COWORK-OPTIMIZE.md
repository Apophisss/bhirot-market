...
> נכתב ב-6.9.2026, אחרי ביקורת עומק של הקוד, הנתונים החיים וצילומי מסך בנייד. המספרים כאן
> נכונים לאותו יום; לפני כל החלטה — משוך נתונים טריים (סעיף 2).
...
| בריאות הלוח: שאלות פתוחות | | 349 | ≤80 | `/api/health`, `markets.health` |
...
| GA4 | הנכס של `G-R9TC3LK5KE` | כל אירוע של האתר מגיע גם לשם (`src/lib/ga-bridge.ts`). Realtime/DebugView = הדרך המהירה לראות שאירוע יורה אחרי פריסה. |
...
- **אף שאלה לא הוכרעה ב-`data/markets.json`** (0 הוכרעו, 17 בוטלו, מהן 16 ככפילויות). השרת מדווח 9 הוכרעו — הקובץ ב-`main` מפגר אחרי השרת, ו-`data/resolutions/` ריק.
...
- **לוח המובילים מדורג לפי שווי כולל** שכולל בונוס הזמנות (500 לחבר, עד 50 חברים = 25,000) — בניגוד לקופי שאומר שהבונוס לא נספר. 5 שחקנים אמיתיים בין 320 מפוברקים, בלי שורת גילוי בדף עצמו.
...
- 349 שאלות פתוחות; **284 בלי אף עסקה**; 59% נסגרות בעוד חודש ומעלה; 105 אחרי יום הבחירות.
- שאלות שנסגרות תוך 72 שעות: 56% מהן נסחרו, 1.13 עסקאות לשאלה. שאלות של 4+ שבועות: 19%, 0.30. **השאלות הקצרות הן המוצר.**
- 81% מהשאלות נסגרות בין 20:00 לחצות; ריצת ההכרעות הראשונה היא ב-10:00 למחרת → 12–20 שעות של ״נסגר ולא הוכרע״.
- `liquidity=2000` בכל שאלה מול תקרת תשובה של 100: להזיז 50%→75% צריך 14 תשובות מקסימליות. המחיר לא מגיב לשחקנים.
...
- סוף ריצה לשחקן מחובר: ״עוד סבב״ / ״לניקוד שלי״ / ״לכל השאלות״. אין ״X מהתשובות שלך מוכרעות עד מחר״, אין הזמנה, אין הצעת התקנה.
...
- **מודאל ״מצב זריז״ חוסם את המסך הראשון בכל ביקור**, כולל שאלה שנפתחה מקישור ב-WhatsApp. הוא גם ה-LCP של דף השאלה (2.4s בנייד מואט).
...
- דף הבית בונה את כל הלוח מ-SQLite בכל בקשה (שלוש סריקות מלאות של `market`), בלי שום cache, על תהליך Node יחיד ב-vCPU אחד: ~0.5s שרת בפרודקשן, ו-8 בקשות במקביל = 619ms כל אחת. זה ה-TTFB (p50 780ms).
...
- אין דפי מועמד/מפלגה — השאילתות שאנשים מקלידים (״נתניהו״, ״איזנקוט״, ״ליכוד״) בלי דף נחיתה. 74 אנשים ב-`data/people.json` עם תמונה וויקי.
...
| G8 | **`price_history`:** גדל ~16k שורות ביום מ-drift; להגביל שאילתות ל-31 יום; אינדקס `(marketId, source, ts)`; גיזום לילי של drift מעל 45 יום. | M | Claude Code | `src/lib/markets.ts`, `src/lib/market-drift.ts`, `drizzle/`, cron | זמן רינדור `/rapid` יציב |
| G9 | **עבודות כבדות מחוץ לתהליך הווב:** המחולל השעתי רץ מקונטיינר `cron` (`npm run markets:generate`), לא בתוך `next start`; `busy_timeout` ל-SQLite; `lb_try_duration` ב-Caddy כדי שפריסה לא תחזיר 502. | M | Claude Code | `docker-compose.yml`, `Caddyfile`, `src/lib/db/index.ts`, `src/app/api/cron/refresh/route.ts` | TTFB p95 |
...
| I7 | **משפטי:** זהות המפעיל + אימייל ב-`/terms` ו-`/privacy` (`CONTACT_EMAIL` ריק היום); לתקן משפט שגוי על הלוח (״השם והתמונה גלויים״ — הלוח אנונימי); סעיפים על מספרי הדגמה ומודל שפה; Consent Mode v2 (ברירת מחדל denied + באנר שורה) — **🛑 הבעלים מחליט על הנוסח והזהות.** | S–M | אתה מכין → 🛑 → Claude Code | `src/app/terms/page.tsx`, `src/app/privacy/page.tsx`, `src/lib/config.ts`, `src/components/GoogleAnalytics.tsx` | |
...
`league`, `suggestion`, `contact`, `bundle_download`, `landing`, `guest_answer`
